...
En la versión original cada `Sum` hacía broadcast a todas las `Aggregation`, lo que implicaba trabajo redundante. Se reemplazó por particionado determinístico: cada `Sum` calcula `SHA-256(fruta) % N` para decidir a qué `Aggregation` enviar cada fruta. Para esto publica en un **direct exchange** con una routing key por instancia de `Aggregation`; cada instancia suscribe únicamente su propia cola. Así, todos los parciales de una misma fruta —provenientes de distintos `Sum`— convergen siempre en la misma `Aggregation`.
...
**Primera solución (descartada): token circulante.** La instancia que consumía el `EOF` emitía sus acumulados, agregaba su ID a una lista `visited` y reinsertaba el token en la misma cola. El proceso se repetía hasta que todas las instancias participaban. Era correcto en términos de orden, pero el cierre era estrictamente secuencial: la latencia total escalaba linealmente con el número de instancias de `Sum`, sumando un round-trip de broker por cada una.

**Solución final: colas de control individuales.** La instancia que consume el `EOF` original publica un aviso de cierre en paralelo a N colas de control dedicadas, una por instancia de `Sum`. Cada instancia escucha su propia cola de control simultáneamente a la cola de datos —el mismo event loop de pika registra ambos consumers en el mismo canal— y al recibir el aviso actúa de forma independiente: emite sus acumulados parciales al exchange y envía un `EOF` a **todas** las `Aggregation`. El cierre ocurre en paralelo con latencia constante independiente de N.

**Trade-off** El aviso de cierre viaja por una cola distinta a los datos. Como el `Gateway` publica a una única cola de entrada (elemento inmutable del sistema), no es posible usar colas individuales por `Sum` para garantizar que el `EOF` llegue en la misma secuencia FIFO que los datos. Esto introduce una race-condition teórica: si el aviso de cierre es despachado por pika antes de que `Sum_Y` consuma sus últimos mensajes de datos pendientes, esos registros se ignoran. En la práctica el aviso debe recorrer un round-trip completo por el broker antes de llegar, tiempo durante el cual `Sum_Y` ya procesó sus mensajes acumulados; el caso patológico requiere una ventana de inactividad muy específica. Como guardia defensiva, los mensajes de datos que llegan tras el cierre de una consulta se descartan explícitamente, y el registro de consultas cerradas se limpia periódicamente para evitar crecimiento ilimitado de memoria.

**Prefetch y su relación con esta race-condition.** El valor de `prefetch_count` configura cuántos mensajes puede pre-entregar RabbitMQ a un consumer antes de recibir un ACK. Con `prefetch=1` el consumer tiene a lo sumo un mensaje en vuelo en cada momento, minimizando la ventana de la race pero con bajo throughput: el consumer queda idle durante cada round-trip al broker. Valores más altos amortizan esa latencia —`prefetch=10` mejora el throughput sensiblemente— pero amplían la ventana: cuando el aviso de cierre llega, pueden existir hasta N mensajes de datos ya pre-entregados en el buffer de pika pero aún no procesados, todos susceptibles de ser desplazados por el aviso si el event loop lo despacha primero. Se mantiene `prefetch=10` como valor intermedio que mejora la performance sin maximizar el riesgo; todos los escenarios de prueba pasan correctamente con este valor dado que el round-trip del aviso de cierre sigue siendo suficiente para que los mensajes pre-entregados sean procesados antes de que el aviso sea atendido.
